# Twinkleswap Profile SDK

This package provides some handy functions to retrieve data for Twinkleswap Profile system.
...
Install `@twinkleswap/profile-sdk` into your project with npm:

```bash
npm install @twinkleswap/profile-sdk --save
```

or yarn:

```bash
yarn add @twinkleswap/profile-sdk
```
...
```js
import TwinkleProfileSdk from "@twinkleswap/profile-sdk";

const twinkleSdk = new TwinkleProfileSdk();
```
...
const twinkleSdk = new TwinkleProfileSdk(customProvider, 97);
```
...
const twinkleSdk = new TwinkleProfileSdk();
const username = twinkleSdk.getUsername("0x123456789");
...
const twinkleSdk = new TwinkleProfileSdk();
const team = twinkleSdk.getTeam(1);
console.log(team);
// {
//   id: 1,
//   name: "Syrup Storm",
...
It also sets `profile_${address}` cookie containing username and avatar (now only for twinkleswap.finance domain, maybe configurable in future versions)

```js
import TwinkleProfileSdk from "@twinkleswap/profile-sdk";

const twinkleSdk = new TwinkleProfileSdk();
const profile = twinkleSdk.getProfile("0x123456789");
console.log(profile);
// {
//   hasRegistered: true
//   profile: {
...
const achievements = twinkleSdk.getAchievements("0x123456789");
...
```JSX
import { Team, teamImages } from "@twinkleswap/profile-sdk";
...
Current version of this SDK is 90% copy of existing from [twinkle-frontend](https://github.com/twinkleswap/twinkle-frontend) repo. There are several improvements to be made in the future versions of this SDK:
